...
def slugify_claude(cwd: str) -> str:
    """Claude Code cwd encoding: / replaced with -, leading - preserved."""
    return cwd.replace("/", "-")
...
def find_claude_session(cwd: str) -> Path | None:
    ...
    if not sessions_dir.is_dir():
        return None
    # Read session metadata to find sessionId for the target cwd
    best_ts = 0
    best_id = None
    ...
    project_jsonl = Path.home() / ".claude" / "projects" / slug / f"{best_id}.jsonl"
    return project_jsonl if project_jsonl.is_file() else None
...
def main() -> int:
    ...
    parser.add_argument("--source", required=True, choices=["codex", "pi", "claude"])
