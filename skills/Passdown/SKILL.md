...
- **If the user specifies a priority (`--priority "fix the auth bug"`)**, bias
  the extraction and summary toward that topic. When compressing, keep turns
  related to the priority even if they'd normally be dropped in the middle.
  The summary should state the priority at the top.
...
```bash
python3 <skill-dir>/scripts/extract_handoff.py --former <codex|pi|claude> --cwd <path>
```

Add `--json` for machine-readable output.

**If the extractor returns "No session found", do not give up.** The script
may have path-matching bugs. Manually locate the session:

- **Claude Code**: `ls ~/.claude/projects/-<slug>/*.jsonl` where slug is the
  cwd with `/` and `_` both replaced by `-`. Pick the most recent by mtime.
- **Codex**: `find ~/.codex/sessions -name "*.jsonl" -newer <yesterday> | sort`
- **Pi**: `ls -t ~/.pi/agent/sessions/--<cwd-with-dashes>--/`

Then run the extractor with `--json` on that direct file path (if the script
doesn't support `--file`, manually read and filter).

### Step 1: Identify former agent and focus

The user specifies which agent to handoff FROM. Two params:

- `--former <codex|pi|claude>` — which agent's session to read. Required if
  ambiguous, optional if obvious from context.
- `--focus "<topic>"` — bias extraction toward this topic. Optional.

Example: `handoff --former codex --focus "database migration"`

When a focus is given, bias extraction toward that topic — keep turns
related to it, drop unrelated tangents first when compressing, and lead
the summary with the focus context.
...
- **Same-agent handoff (Claude→Claude, Codex→Codex, Pi→Pi)**: The extractor
  reads the most recent session for the cwd, including the currently active
  one. The session JSONL is written in real-time — the new conversation gets
  compressed context from the previous session. No special flags needed.
...
- **Claude Code slug**: The project directory slug replaces BOTH `/` and `_`
  with `-`. If the extractor returns "No session found", manually check
  `ls ~/.claude/projects/-<cwd-slug>/` (replace `/` and `_` with `-`).
