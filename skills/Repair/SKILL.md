...
description: |
  ...
  `/media/yhr/2T/yunxiao/bugs/<bug-id>/` 的缺陷单，三阶段：Intake
  读取缺陷单、切输入分支、结合代码分析、生成 fix_plan.md 和 Breach
  对齐页、将状态改为 修复中；Fix 按修复方案和 Neutralize 工作流修复，按条件
  触发 Codify 和 AfterAction；Closeout 更新评论区并只修改缺陷状态。
...
一个 skill，三个 mode。缺陷单原始上下文由共享 Phase 0 生成，Repair 只负责接单分析、修复和收尾。

## 路由

| Mode | 触发 | 文件 |
|------|------|------|
| `Intake` | `/Repair Intake <bug-id> --branch <branch>` 或 `/Repair Intake <bug-id> --base <branch>` | `intake.md` |
...
- **状态集合**: `待处理`、`重新打开`、`修复中`、`开发挂起`、`回归验证`、`转需求`、`关闭`
- **负责人规则**: 全流程不修改负责人；只读负责人字段，只修改评论区和缺陷状态
- **分支规则**: `--branch` 只切已有分支；`--base` 从 base 创建 `bugfix/<bug-id>`；dirty worktree 停止，不自动 stash
